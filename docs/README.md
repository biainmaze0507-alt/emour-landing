...
| `brand.html` | `brand` | 이름의 뜻 · 디자인 모티프 · 컬러 아이덴티티 · 웜톤/쿨톤 · 심볼 설계도 · 타이포 |
| `team.html` | `team` | 팀원 6명 |
...
(HEX 값은 `css/00-tokens.css` 와 `js/data/emotions.js` 두 곳에 있으니 함께 맞춰 주세요.)
...
- `MOTIF` — 디자인 모티프와 단색 원 세 개
- `ORIGIN_PALETTE` — 색마다 담긴 **뜻**
- `TEMPERATURE` — 웜톤 / 쿨톤 두 축
- `LOGO_BUILD` — 심볼 설계도(격자 보조선 · 설명 네 줄)
...
| 홈 히어로 | 입력창에 글자가 쳐지고 → 말풍선으로 보내지고 → AI 분석 → 감정 태그 → 추천 칩 순서로 반복 재생 (창 높이는 고정) |
| 홈 기획 배경 | **감정 단서 ON/OFF 스위치** — 같은 대화가 무채색 ↔ 감정 표시로 전환 |
| 제품 · 핵심 기능 | 탭 3개 전환 (마우스 · ← → Home End 키) · 화면이 실시간 교체 |
| 제품 · 감정 시스템 | 극성 필터 4종 · 라벨 무한 흐름 띠 |
...
| 브랜드 · 심볼 | 격자 위에 보조선을 깔고 실제 심볼을 얹은 설계도 · 바로 아래 기본형(심볼+워드마크) |
| 브랜드 · 팔레트 | 여덟 색이 헤어라인으로 끊긴 한 줄씩, 색마다 담긴 **뜻**을 함께 적음 |
| 전역 | 스크롤 진행바 · 우측 하단 맨 위로 버튼 · 커서 글로우 · 등장 연출 |
